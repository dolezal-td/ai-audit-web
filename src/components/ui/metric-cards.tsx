...
export function MetricCards({ data }: MetricCardsProps) {
  return (
    <div className="grid grid-cols-2 gap-3 my-8 sm:grid-cols-3 lg:grid-cols-5">
      {data.map((metric) => {
        ...
        return (
          <div
            key={metric.title}
            className="rounded-xl border bg-fd-card p-4 flex flex-col gap-2.5"
          >
            <div className="flex items-center justify-between gap-1">
              <span className="text-xs font-medium text-fd-muted-foreground leading-tight">
                {metric.title}
              </span>
              {metric.description && (
